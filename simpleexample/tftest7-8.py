"""
对异或数据进行分类
1.基于退化学习率来改善训练时的抖动情况
2.通过正则化和dropout改善过拟合情况
3.正则化：L2范数
4.dropout: 丢弃40%的结点
5.200个神经元
"""
...
# 使用退化学习率动态调整学习率大小，从而改善训练时的抖动情况
learning_rate = 0.01
global_step = tf.Variable(0, trainable = False)
learning_rate_decay = tf.train.exponential_decay(learning_rate, global_step, 1000, 0.9)
n_input  = 2
n_label  = 1
...
keep_prob = tf.placeholder(tf.float32)  # 在sess.run()时指定留下的节点比例
layer_1_dropout = tf.nn.dropout(layer_1, rate = 1 - keep_prob)  # 在layer_1后面添加一层dropout 

z2 = tf.matmul(layer_1_dropout, weights["h2"]) + biases["h2"]
...
optimizer = tf.train.AdamOptimizer(learning_rate_decay).minimize(cost, global_step = global_step)  # 优化器用Adam
...
for epoch in range(training_epochs):
    _, lossval = sess.run([optimizer, cost], feed_dict = {X: train_X, Y: train_Y, keep_prob: 0.6})  # keep_prob:0.6表示在dropout时丢弃40%的结点
...
print ("loss:\n", sess.run(cost, feed_dict={X: validate_X, Y: validate_Y, keep_prob: 1.0}))   
