...
import speech_recognition.utils as speech_utils

wav_path = './data/wav/train'
label_file = './data/doc/trans/train.word.txt'

wav_files, labels = speech_utils.get_wavs_lables(wav_path, label_file)
...
def next_batch(labels, start_idx = 0, batch_size = 1, wav_files = wav_files):
    ...
    (source, audio_len, target, transcipt_len) = speech_utils.get_audio_and_transcriptch(None, wav_files, n_input, n_context, word_num_map, txt_labels)
    start_idx += batch_size
    if start_idx >= filesize:
        start_idx = -1

    source, source_lengths = speech_utils.pad_sequences(source)  # 如果有多个文件将长度统一，支持按最大截断或补0
    sparse_labels = speech_utils.sparse_tuple_from(target)
...
t = speech_utils.sparse_tuple_to_texts_ch(sparse_lab, words)
print(t[0])
...
def BiRNN_model(batch_x, seq_length, n_input, n_context, n_character, keep_dropout):
     ...
     # 将2维[n_steps*batch_size, n_character]转成3维time-major [n_steps, batch_size, n_character]
...
# input_tensors是输入的语音MFCC features,它的的shape是[batch_size, max_stepsize, n_input + (2*n_input*n_context)]
input_tensor = tf.placeholder(tf.float32, [None, None, n_input + (2*n_input*n_context)], name = 'input')
# targets是输入语音对应文本
# ctc_loss计算时需要使用sparse_placeholder来生成SparseTensor
targets = tf.sparse_placeholder(tf.int32, name = 'targets')
# 序列长，1d array of size [batch_size]
seq_length = tf.placeholder(tf.int32, [None], name = 'seq_length')
keep_dropout = tf.placeholder(tf.float32)


# logits是BiRNN的最后一层layer，同时也是ctc loss的输入
...
with tf.Session() as sess:
    ...
    for epoch in range(epochs):
        ...
        for batch in range(n_batches_per_epoch):
            ...
            feed_dict = {input_tensor: source, targets: sparse_labels, seq_length: source_lengths, keep_dropout: keep_dropout_rate}

            # 计算avg_loss optimizer
            batch_cost, _ = sess.run([avg_loss, optimizer], feed_dict = feed_dict)
            train_cost += batch_cost

            if (batch + 1) % 20 == 0:
                print('loop:',batch, ' Train cost: ', train_cost/(batch + 1))
                feed_dict = {input_tensor: source, targets: sparse_labels, seq_length: source_lengths, keep_dropout: 1.0}
                d, train_ler = sess.run([decoded[0], ler], feed_dict = feed_dict)
                dense_decoded = tf.sparse_tensor_to_dense(d, default_value = -1).eval(session = sess)
                dense_labels = speech_utils.sparse_tuple_to_texts_ch(sparse_labels, words)

                counter = 0
                print('Label err rate: ', train_ler)
                for orig, decoded_arr in zip(dense_labels, dense_decoded):
                    # 转成strings
                    decoded_str = speech_utils.ndarray_to_text_ch(decoded_arr, words)
        ...
        log = 'Epoch {}/{}, train_cost: {:.3f}, train_ler: {:.3f}, time: {:.2f} sec'.format(epoch, epochs, train_cost, train_ler, epoch_duration)
        print(log)
        saver.save(sess, savedir + 'speech_recognition.ckpt', global_step=epoch)

    train_duration = time.time() - train_start
    print('Training complete, total duration: {:.2f} min'.format(train_duration))
