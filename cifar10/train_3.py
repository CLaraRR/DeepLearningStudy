...
5.多通道卷积技术：定义多个不同尺寸的卷积核，将卷积后的结果连接起来
6.在卷积后添加Batch Normalization
'''
...
# 定义第二个卷积层、最大池化层,定义多个不同尺寸（1x1, 3x3, 5x5, 7x7）的卷积核，然后将卷积结果连接起来
W_conv2_1x1 = weight_variable([1, 1, 64, 64])  # 定义卷积核的高度、宽度、图像通道数（上一层输出个数）、输出的feature map个数
b_conv2_1x1 = bias_variable([64])  # 定义偏置值的个数和卷积核的输出个数一样
...
h_conv2_1x1 = tf.nn.relu(batch_norm_layer(conv2d(h_pool1, W_conv2_1x1) + b_conv2_1x1, is_train = is_train))  # 上一层的输出经过卷积操作，然后还要用relu函数来激活
h_conv2_3x3 = tf.nn.relu(batch_norm_layer(conv2d(h_pool1, W_conv2_3x3) + b_conv2_3x3, is_train = is_train))
h_conv2_5x5 = tf.nn.relu(batch_norm_layer(conv2d(h_pool1, W_conv2_5x5) + b_conv2_5x5, is_train = is_train))
h_conv2_7x7 = tf.nn.relu(batch_norm_layer(conv2d(h_pool1, W_conv2_7x7) + b_conv2_7x7, is_train = is_train))
h_conv2 = tf.concat([h_conv2_1x1, h_conv2_3x3, h_conv2_5x5, h_conv2_7x7], 3)  # 将卷积结果（在channels这个维度上）连接起来

h_pool2 = max_pool_2x2(h_conv2)  # 激活后进入最大池化层

# 定义第三个（最后）卷积层、均值池化层
W_conv3 = weight_variable([5, 5, 256, 10])  # 定义卷积核的高度、宽度、图像通道数（上一层输出个数）、输出的feature map个数（类别数）
