...
h_conv1 = tf.contrib.layers.conv2d(x_image, 32, 5, 1, 'SAME', activation_fn = tf.nn.relu)  # 原始图像经过卷积操作，然后还要用relu函数来激活
...
h_conv3 = tf.contrib.layers.conv2d(h_pool2, 10, [5, 5], 1, 'SAME', activation_fn = tf.nn.relu)  # 上一层的输出经过卷积操作，然后还要用relu函数来激活

nt_hpool3 = tf.contrib.layers.avg_pool2d(h_conv3, [7, 7], stride = 7, padding = 'SAME')  # 激活后进入均值池化层
...
savedir = "./mnist_classify/model_cnn2/"
